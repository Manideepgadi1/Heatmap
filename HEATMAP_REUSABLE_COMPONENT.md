# Reusable Heatmap Component Package

This document contains all the code needed to implement the heatmap in another project.

---

## 1. HEATMAP COMPONENT (Heatmap.jsx)
...
const Heatmap = ({ 
  ...
  avgMonthlyProfits3y, 
  rankPercentile4y, 
  inverseRankPercentile, 
  monthlyRankPercentile 
}) => {
...
The heatmap expects this data structure:

```javascript
// Example of expected props
const exampleProps = {
  indexName: "NIFTY 50",
  
  // Heatmap data: year -> month -> value
  heatmapData: {
    "2025": {
      "1": 0.0235,    // January: +2.35%
      "2": 0.0187,    // February: +1.87%
      "3": -0.0123,   // March: -1.23%
      // ... up to "12"
    },
    "2024": {
  ...
  // Monthly prices: year -> month -> price
  monthlyPrice: {
    "2025": {
      "1": 38380.50,
      "2": 39100.00,
  ...
  // Monthly profits (same structure as heatmapData)
  monthlyProfits: {
    "2025": {
      "1": 0.0235,
      // ...
  ...
  // Single values
  avgMonthlyProfits3y: 0.0068,        // 0.68%
  rankPercentile4y: 70.8,              // 70.8%
  inverseRankPercentile: 29.2,         // 29.2%
  
  // Monthly rank positions: year -> month -> rank number
  monthlyRankPercentile: {
    "2025": {
      "1": 15,      // Rank #15
      "2": 4,       // Rank #4
...
### Change Colors
Modify the `getColor` function to use your own color scheme.

### Remove Metric Cards
Delete or comment out the metric cards section if you only want the heatmap grid.

### Change Cell Size
Modify `width: 70` and `height: 50` in the cell Box components.

### Disable Download
Remove the download button and `handleDownload` function.

### Remove Rankings
Don't pass `monthlyRankPercentile` prop or modify the cell rendering to exclude rank display.
...
✅ Color-coded heatmap grid (red to green)
✅ 5 metric cards with gradients
...
Copy the Heatmap.jsx code to your new project and ensure you have the required dependencies installed!
