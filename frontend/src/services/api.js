...
// Use /api/heatmap for production, localhost:8000 for development
const API_BASE_URL = import.meta.env.DEV ? 'http://localhost:8000' : '/api/heatmap';
 ...
 * @param {string} trailingPeriod - Optional trailing period ('1M', '3M', '6M', '1Y', '2Y', '3Y', '4Y')
 * @returns {Promise<Object>} Heatmap data with structure: { index, heatmap }
 * @throws {Error} If the request fails
 */
export const fetchHeatmap = async (indexName, forwardPeriod = null, trailingPeriod = null) => {
  try {
    const params = {};
    if (forwardPeriod) params.forward_period = forwardPeriod;
    if (trailingPeriod) params.trailing_period = trailingPeriod;
